...
class Vamtam_Import extends WP_Importer {
	...
	function import_end() {
		...
		if ( $this->one_click ) {
			...
			if ( current_theme_supports( 'vamtam-foodpress-import' ) ) {
				esc_html_e( 'Will process FoodPress ids for the following pages:', 'wordpress-importer' );
				...
				foreach ( $pages as $page ) {
					if ( strpos( $page->post_content, '[add_foodpress' ) !== false ) {
						echo $page->ID . ', ';

						$page->post_content = preg_replace_callback( '/(\[add_foodpress_menu_item(?:(?!item_id=").)+item_id=")([^"]+)/', array( $this, 'fp_menu_item' ), $page->post_content );

						$page->post_content = preg_replace_callback( '/(\[add_foodpress_menu(?:(?!dish_type=").)+dish_type=")([^"]+)/', array( $this, 'fp_menu' ), $page->post_content );
	...
	public function fp_menu_item( $matches ) {
		// 11690
		return $matches[1] . $this->processed_posts[ intval( $matches[2] ) ];
	}

	public function fp_menu( $matches ) {
		// 511, 533
		$dish_types = explode( ',', $matches[2] );

		foreach ( $dish_types as &$dish_type ) {
			$dish_type = $this->processed_terms[ intval( trim( $dish_type ) ) ];
		}

		return $matches[1] . implode( ',', $dish_types );
	}
