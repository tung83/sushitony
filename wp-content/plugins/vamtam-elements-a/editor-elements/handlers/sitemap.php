...
class Vamtam_Sitemap {
	...
	private function categories( $atts ) {
		...
		$feed = ($show_feed === true || $show_feed == 'true') ? esc_html__( 'RSS', 'wpv' ) : '';
	...
	private function projects( $atts ) {
		extract(shortcode_atts(array(
			'show_comment'   => false,
			'number'         => '0',
			'portfolio_type' => '',
		...
		if ( ! empty( $portfolio_categories ) ) {
			$query['taxonomy'] = 'jetpack-portfolio-type';
	...
	private function display( $query, $show_comment = false ) {
		...
		while ( $archive_query->have_posts() ) {
			...
			if ( ! empty( $title ) ) {
				$output .=
					"<li>
						<a href='" . get_permalink() . "' rel='bookmark' title='" . esc_attr( sprintf( __( 'Permanent Link to %s', 'wpv' ), $title ) )."'>".
							strip_tags( $title ) . '</a>'.
						($show_comment?' ('.get_comments_number().')':''); if ( $post->post_type == 'page' ) {
	...
	private function all( $atts ) {
		extract(shortcode_atts(array(
			'number' => '0',
			'shows'  => 'pages,categories,posts,projects',
		), $atts));
		...
		foreach ($types as $type => $name)
			if ( in_array( $type, $shows ) ) {
				$output .= Vamtam_Text_Divider::shortcode(array(
					'more' => '',
					'type' => 'double',
				), $name);
