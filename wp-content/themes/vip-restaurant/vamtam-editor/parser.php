			...
			if ( $this->content[ $pos ] == '[' ) {
				...
				if ( $this->content[ $pos + 1 ] === '/' ) {
					...
					if ( $type != $this->ptr->type ) {
						...
						throw new Exception( "<!-- vamtam editor error --><div class='vamtam-editor-error'><span style='color: red'>Improperly closed shortcodes. Expected <strong>[/{$this->ptr->type}]</strong>, but found <strong>[/{$type}]</strong> on position ". ($pos - strlen( $type ) - 2 .' (character '.$char.'; line '.$line.")</span><br><br>Context: $context</div>") );
					}
