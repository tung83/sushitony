...
(function($, undefined) {
	...
	$(function() {
		ED.container = $('#vamtam_visual_editor');
		ED.savingBlocked = true;

		if (ED.container.length === 1) {
			var editor_button = $('<a class="vamtam-editor-toggle wp-switch-editor">Vamtam</a>');
			...
			if ($('#vamtam_ed_js_status').val() === 'true') editor_button.click();
	...
	var ED = {
		init: function() {
			...
			ED.toolbox = $('#vamtam-editor-toolbox');
		...
		controls: function() {
			...
			$('body').on('click', '.vamtam-save-element', function(e) {
				ED.edit_form.save();
				e.preventDefault();
			});

			$('body').on('click', '.vamtam-cancel-element', function(e) {
				ED.edit_form.cancel();
				e.preventDefault();
			});
		...
		get_editor_html: function(refresh) {
			...
			$.ajax({
				...
				url: ajaxurl,
				data: {
					action: 'vamtam_editor_init_html',
					content: content
				},
				success: function(data) {
					...
					ED.container.find(".vamtam-editor-error").on("selectstart mousedown mousemove", function(e) {
						e.stopPropagation();
						return true;
					});
					ED.container.find(".vamtam-ed-param-holder.html-content").bind("mousedown", function(e) {
						e.stopPropagation();
					});
				}
			});
		...
		dragdrop: {
			init: function() {
				$('.droppable_source', ED.container).draggable({
					...
					start: function(event) {
						...
						$('.vamtam-dd-active').removeClass('vamtam-dd-active');
			...
			refresh: function() {
				ED.rowClasses($(".vamtam_main_sortable"));

				try {
					$('.vamtam_sortable.ui-draggable', ED.container).draggable('destroy');
					$('.inner-sortable.ui-droppable', ED.container).droppable('destroy');
				} catch(e) {}

				$('.vamtam_sortable', ED.container).draggable({
					appendTo: ED.content,
					handle: '> .controls',
					scroll: true,
					zIndex: 1000,
				...
				$('.inner-sortable', ED.container).droppable({
					tolerance: 'pointer',
					greedy: true,
					accept: '.vamtam_sortable, .droppable_source',
					hoverClass: "ui-state-active",
					over: function() {
						$(this).addClass('vamtam-dd-active');
					},
					out: function() {
						$(this).removeClass('vamtam-dd-active');
					},
			...
			insert: function(draggable, droppable, d_offset) {
				var elements = droppable.find('>.vamtam_sortable'),
					method = 'after',
					toEl = false,
					offsets = [],
					max_offset_top = 0;
				...
				if(elements.length > 0) {
					...
					if(offsets.length === 0) {
						toEl = droppable.find('.vamtam_sortable:first');
						method = 'before';
					} else {
			...
			empty_notice: function() {
				...
				sortables.append('<span class="'+span_class+'">'+VAMTAMED_LANG.empty_notice+'</span>');
		...
		elementButtons: function() {
			$('.clickable_action', ED.container).click(function(e) {
				...
				if(!ED.creteTarget)
					ED.createTarget = $(".vamtam_main_sortable", ED.container);
		...
		saveHTML: function(callback) {
			...
			if ($(".vamtam_main_sortable").html().indexOf('<!-- vamtam editor error -->') > -1) {
				console.log("ED.saveHTML can't save - there's a parser error");
				return;
			}

			var shortcodes = ED.generate_shortcodes($(".vamtam_main_sortable"));
		...
		createElement: function(target, element, action, callback) {
			...
			$.ajax({
				...
				success: function(data) {
					...
					data.find(".vamtam-ed-param-holder.html-content").bind("mousedown", function(e) {
						e.stopPropagation();
					});
		...
		tinymce: {
			init: function(element) {
				var id = element.attr("id"),
					wrap = element.closest('.vamtam-config-row').find('.wp-editor-wrap'),
					original_id = 'content';
				...
				try {
					...
					wrap.bind('click.vamtam-active-editor', function() {
						setTimeout(function() {
							wpActiveEditor = id;
						}, 100);
					});
		...
		edit_form: {
				...
				$.ajax({
					...
					success: function(response) {
						...
						$('body')
							.vamtamColorPicker()
							.vamtamIconsSelector()
							.vamtamBackgroundOption();

						$(">.vamtam-ed-param-holder", element).each(function() {
							...
							if ($(this).hasClass('toggle') || $(this).hasClass('radio') || $(this).hasClass('icons')) {
								...
								param.val(new_value);
								$.VAMTAM.upload.fill(param.attr('id'), new_value);
						...
						ED.form.find('.vamtam-range-input').uirange();
					}
				});
			},
			...
			save: function() {
				...
				ED.form.find('[name]').filter('input, textarea, select').each(function() {
					var new_value = '',
						name = $(this).attr("name"),
						holder = element.find('> .vamtam-ed-param-holder.' + name.replace('[]', ''));
				...
				$($.unique(multicb)).each(function() {
					var holder = element.find('.vamtam-ed-param-holder.' + this.replace('[]', ''));
					var new_value = [];
		...
		generate_shortcodes: function(root, level) {
			...
			columns.each(function() {
				...
				element.children('.vamtam-ed-param-holder:not(.html-content, .noattr)').each(function() {
				...
				var wrap_before = '',
					wrap_after = '',
					column_title = element.find('> .vamtam-ed-param-holder[name="column_title"]').val(),
					column_title_type = element.find('> .vamtam-ed-param-holder[name="column_title_type"]').val(),
					column_divider = element.find('> .vamtam-ed-param-holder[name="column_divider"]').val(),
					column_animation = element.find('> .vamtam-ed-param-holder[name="column_animation"]').val();

				if (element.hasClass('vamtam_ed_column')) {
				...
				output += ED.apply_filters(element, 'generated-shortcode', {
					shortcode: ( wrap_before + column_output + wrap_after ).replace( /\n\n+/, '\n\n' ),
					level: level
				}).shortcode;
			});
		...
		shortcodes: {
			...
			services: {
				init: function() {
					ED.add_callback('init-expandable-services', function() {
						...
						$(this).find('.vamtam-ed-param-holder.closed').val(content[0]);
						$(this).find('.vamtam-ed-param-holder.html-content').html(content[1]);
					});
					...
					ED.add_callback('generate-expandable-services', function(action, data) {
						...
						return {
							shortcode: data.shortcode.replace(/(\[services_expandable[^\]]+\])/, function(a) {
								return a + container.find('.vamtam-ed-param-holder.closed').val() + '\n[split]\n' + container.find('.vamtam-ed-param-holder.html-content').val() + "\n";
							}),
							level: data.level
						};
					});
			...
			tabs: {
				init: function() {
					...
					ED.add_callback('generate-tabs', function(action, data) {
						var container = $(this).find('.vamtam_tabs');
					...
					$('body').on('click', '.tab-add', function(e) {
						...
						li.before('<li>\
								...
								<a class="vamtam-icon-selector-trigger tab-icon-selector vamtam-icon no-icon" title="Change Icon" data-icon-name="">&nbsp;</a>\
							</li>');

						li.closest('.vamtam_tabs').append('<div id="tabs-' + suffix + count + '" class="clearfix inner-sortable"></div>').tabs('destroy').tabs();
					...
					$('body').on('keypress blur paste', '.vamtam_tabs textarea', function() {
						setTimeout(ED.saveHTML, 50);
					});
				...
				reload: function() {
					$('.vamtam_tabs', ED.container).tabs();
					$('.vamtam_tabs > ul', ED.container).sortable({
			...
			accordion: {
				init: function() {
					ED.add_callback('init-accordion', function() {
						if ($(this).find('.accordion-add').length === 0) {
							$(this).append(function() {
								return '<div class="vamtam_accordion">'+
										'<div>\
					...
					ED.add_callback('generate-accordion', function(action, data) {
						var container = $(this).find('.vamtam_accordion');
					...
					$('body').on('keydown change paste blur', '.vamtam_accordion textarea', function() {
						setTimeout(ED.saveHTML, 50);
					});
					...
					$('body').on('click', '.accordion-add', function(e) {
						...
						block.closest('.vamtam_accordion').accordion('destroy');
				...
				reload: function() {
					$('.vamtam_accordion', ED.container).accordion({
