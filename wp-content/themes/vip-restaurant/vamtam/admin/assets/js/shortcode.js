var vamtam_shortcode_last_update = 0;
...
(function($, undefined) {
	...
	var shortcode = {
		preview: function() {
			if (Date.parse(new Date()) - vamtam_shortcode_last_update > 1000) {
				vamtam_shortcode_last_update = Date.parse(new Date());
		...
		init: function() {
			$('#shortcodes .vamtam-range-input').uirange();
		...
		fields: {
			...
			button: {
				atts: ['id', 'style', 'class', 'align', 'link', 'linkTarget', 'bgcolor', 'text_color', 'hover_color', 'font', 'icon', 'icon_placement', 'icon_color'],
				content: 'text'
			},
			chart: {
		...
		generate: function() {
			var type = $('body').attr('data-vamtamshortcode');
	...
	$(window).bind('vamtam_shortcodes_loaded', function() {
		shortcode.init();
	});
