 ...
 */
/**
 * class VamtamIconsHelper
 */
class VamtamIconsHelper extends VamtamAjax {
	...
	public static function process_icon_font() {
		...
		if ( $res === true ) {
			...
			$selection = json_decode( vamtam_silent_get_contents( self::$storage_path . 'temp/selection.json' ) );
	...
	public static function admin_menu() {
		add_theme_page( esc_html__( 'VamTam Icons', 'wpv' ), esc_html__( 'VamTam Icons', 'wpv' ), 'edit_theme_options', 'vamtam_icons', array( __CLASS__, 'page' ) );
	}

	public static function page() {
		include VAMTAM_ADMIN_TEMPLATES . 'icons-manager.php';
	}
	...
	public function get_icon_list() {
		...
		$icons  = vamtam_get_icons_extended();
		$result = array();

		$result[''] = '<span>'.esc_html__( 'No icon', 'wpv' ).'</span>';

		foreach ( $icons as $key => $name ) {
			$type = vamtam_get_icon_type( $key );

			if ( $type !== '' ) {
				$type .= '-icon';
			}

			$result[ $key ] = '<span title="'.esc_attr( $name ).'" class="vamtam-icon '. $type . '">&#' . vamtam_get_icon_num( $key ) . '</span>';
		}
