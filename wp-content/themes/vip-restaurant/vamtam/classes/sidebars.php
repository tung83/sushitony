...
class VamtamSidebars {
	...
	public function __construct() {
		$this->sidebars = array(
			'page' => esc_html__( 'Main Widget Area', 'wpv' ),
		);

		if ( vamtam_has_woocommerce() )
			$this->sidebars['vamtam-woocommerce'] = esc_html__( 'WooCommerce Widget Area', 'wpv' );
		...
		foreach ( $this->sidebars as $id => $name ) {
			foreach ( $this->places as $place ) {
				register_sidebar( array(
					...
					'before_title'  => apply_filters( 'vamtam_before_widget_title', '<h4 class="widget-title">', 'body' ),
					'after_title'   => apply_filters( 'vamtam_after_widget_title', '</h4>', 'body' ),
				) );
			}
		}
		...
		for ( $i = 1; $i <= 8; $i++ ) {
			register_sidebar(
				array(
					...
					'before_widget' => '<section id="%1$s" class="widget %2$s">',
					'after_widget'  => '</section>',
					'before_title'  => apply_filters( 'vamtam_before_widget_title', '<h4 class="widget-title">', 'footer' ),
					'after_title'   => apply_filters( 'vamtam_after_widget_title', '</h4>', 'footer' ),
				)
			);
		}
		...
		for ( $i = 1; $i <= 8; $i++ ) {
			register_sidebar(
				array(
					...
					'before_widget' => '<section id="%1$s" class="widget %2$s">',
					'after_widget'  => '</section>',
					'before_title'  => apply_filters( 'vamtam_before_widget_title', '<h4 class="widget-title">', 'header' ),
					'after_title'   => apply_filters( 'vamtam_after_widget_title', '</h4>', 'header' ),
				)
			);
		}
	}
