...
module.exports = function(grunt) {
	...
	grunt.registerTask('download-sidebars-options', function() {
		...
		var parts = [
			...
			['default-options-redux', 'default-options.php', 'options'],
		], pi = -1;
							...
							for(var prop in skin_data) {
								for(var ii in images) {
									if(typeof skin_data[prop] === "string" && skin_data[prop].match(images[ii])) {
										skin_data[prop] = skin_data[prop].replace(images[ii], '{VAMTAM_SAMPLES_URI}images/' + path.basename(images[ii]));
									}
								}
							}
		...
		for(var fi in files) {
			grunt.file.write(files[fi],
				grunt.file.read(files[fi])
					.replace( /,\s*(['"])(vamtam)\1/g, ", '"+theme_name+"'")
					.replace( /(load_theme_textdomain|is_textdomain_loaded)\(\s*'(vamtam)'/g, "$1( '"+theme_name+"'")
			);
		}
	});
	...
	grunt.registerTask('download-lessphp', function() {
		...
		var classes = [
			['lessc', 'VamtamLessc'],
			['lessc_parser', 'VamtamLesscParser'],
			['lessc_formatter_classic', 'VamtamLesscFormatter_classic'],
			['lessc_formatter_compressed', 'VamtamLesscFormatter_compressed'],
			['lessc_formatter_lessjs', 'VamtamLesscFormatter_lessjs'],
		];
		...
		var rename_strings = function() {
			...
			contents = contents.replace(/lessc_formatter_lessjs/g, 'VamtamLesscFormatter_lessjs');
			...
			contents = contents.replace(/(['"'])(lessc\w*)/g, function(match, quot, name) {
				return quot + 'Vamtam' + name[0].toUpperCase() + name.replace(/_([a-z])/g, function(m) { return m[1].toUpperCase(); }).slice(1);
			});
