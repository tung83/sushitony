...
?>
		<li>
			<a href="<?php echo esc_url( $link ) ?>" rel="nofollow" target="_blank" title="<?php echo esc_attr( $name )?>">
				<span class="icon before <?php echo esc_attr( vamtam_get_icon_type( $icon ) ) ?>" data-icon-type="<?php echo esc_attr( $icon ) ?>"><?php vamtam_icon( $icon )?></span><span class="content"><?php echo wp_kses_post( $name ) ?></span></a>
		</li>
<?php endfor ?>
</ul>
